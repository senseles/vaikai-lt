...
export default async function KindergartenPage({ params }: PageProps) {
  const cityName = CITY_NAMES[params.city];
  if (!cityName) notFound();
  ...
  const typeBadge = badgeMap[item.type] ?? { label: item.type, cls: 'bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300' };
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 py-6 sm:py-8">
      ...
      <nav className="text-sm text-gray-500 dark:text-gray-400 mb-6" aria-label="Navigacija">
        ...
        <span className="mx-2">/</span>
        <Link href={`/${params.city}?category=darzeliai`} className="hover:text-primary transition-colors">Darželiai</Link>
        <span className="mx-2">/</span>
        ...
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            <div className="flex items-center gap-3 flex-wrap">
              ...
              <span className={`text-xs px-3 py-1 rounded-full font-medium shrink-0 ${typeBadge.cls}`}>{typeBadge.label}</span>
            </div>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-100 dark:border-slate-700 p-5 sm:p-6 mb-6">
        ...
        <dl className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {item.address && <InfoItem label="Adresas" value={item.address} />}
          {item.phone && <InfoItem label="Telefonas" value={item.phone} />}
          {item.website && <InfoItem label="Svetainė" value={item.website} link />}
          {item.language && <InfoItem label="Kalba" value={item.language} />}
          {item.hours && <InfoItem label="Darbo laikas" value={item.hours} />}
          {item.ageFrom != null && <InfoItem label="Amžius nuo" value={`${item.ageFrom} m.`} />}
          {item.groups != null && <InfoItem label="Grupės" value={String(item.groups)} />}
        ...
        {features.length > 0 && (
          <div className="mt-4 pt-4 border-t border-gray-100 dark:border-slate-700">
            <p className="text-xs text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-2">Ypatybės</p>
            <div className="flex flex-wrap gap-2">
              {features.map((f: string) => (
                <span key={f} className="text-xs bg-green-50 dark:bg-green-900/30 text-green-700 dark:text-green-400 px-2.5 py-1 rounded-full">{f}</span>
              ))}
            </div>
          </div>
        )}
      ...
      <div className="mt-8 pt-6 border-t border-gray-200 dark:border-slate-700">
        <Link
          href={`/${params.city}?category=darzeliai`}
          className="inline-flex items-center gap-2 text-primary hover:text-primary-dark font-medium transition-colors"
        >
          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" />
          </svg>
          Grįžti į {cityName} darželių sąrašą
        </Link>
      </div>
    </div>
  );
