...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const title = `${item.name} — auklė ${cityName} | Vaikai.lt`;
  const description = item.description
    ? item.description.slice(0, 160)
    : `${item.name} — auklė ${cityName} mieste. Atsiliepimai, kontaktai ir vertinimai.`;
  const url = `https://vaikai.lt/${params.city}/aukles/${params.slug}`;
...
export default async function AuklePage({ params }: PageProps) {
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: 'Auklės', item: `https://vaikai.lt/${params.city}?category=aukles` },
