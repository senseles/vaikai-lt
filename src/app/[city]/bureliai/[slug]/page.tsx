...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const title = `${item.name} — būrelis ${cityName} | ManoVaikai.lt`;
  const description = item.description
    ? item.description.slice(0, 160)
    : `${item.name}${item.category ? ` (${item.category})` : ''} — būrelis ${cityName} mieste. Atsiliepimai ir vertinimai.`;
  const url = `https://manovaikai.lt/${params.city}/bureliai/${params.slug}`;
...
export default async function BurelisPage({ params }: PageProps) {
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: 'Būreliai', item: `https://manovaikai.lt/${params.city}?category=bureliai` },
    ...
    <div className="max-w-4xl mx-auto px-4 sm:px-6 py-6 sm:py-8">
      ...
      <div className="mb-6">
        <div className="flex items-start justify-between gap-3">
          ...
          <div className="flex items-center gap-2 shrink-0"><SuggestEditButton entityId={item.id} entityType="burelis" entityName={item.name} currentData={item as unknown as Record<string, unknown>} /><FavoriteButton itemId={item.id} itemType="burelis" /></div>
        </div>
