...
/** GET /api/admin/aukles — list with search/pagination/sort */
export async function GET(request: NextRequest) {
  ...
  const validSortFields = ['name', 'city', 'baseRating', 'baseReviewCount', 'experience', 'createdAt'];
  const sortField = validSortFields.includes(sort) ? sort : 'name';
  const sortDir = dir === 'desc' ? 'desc' as const : 'asc' as const;
  const orderBy = { [sortField]: sortDir };

  const [items, total] = await Promise.all([
    prisma.aukle.findMany({ where, orderBy, skip, take: limit }),
...
/** POST /api/admin/aukles — create new aukle */
export async function POST(request: NextRequest) {
  try {
    ...
    const optionalStrings = ['phone', 'email', 'experience', 'ageRange', 'hourlyRate', 'languages', 'description', 'availability', 'region', 'area'];
    for (const key of optionalStrings) {
      if (body[key] !== undefined && body[key] !== '') data[key] = String(body[key]).trim();
    }

    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const created = await prisma.aukle.create({ data: data as any });
    return json({ success: true, data: created }, 201);
  } catch (err) {
    console.error('Admin create aukle error:', err);
