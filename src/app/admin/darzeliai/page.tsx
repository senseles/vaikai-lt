...
const columns: ColumnDef[] = [
    ...
    render: (val) => {
      ...
      return (
        <span className={`text-xs px-2 py-1 rounded-full font-medium ${
          t === 'privatus' ? 'bg-purple-50 text-purple-700' : 'bg-blue-50 text-blue-700'
        }`}>
  ...
  },
  {
    key: 'baseReviewCount',
    label: 'Atsiliepimai',
    render: (val) => <span className="text-gray-500 text-sm">{Number(val ?? 0)}</span>,
    hideOnMobile: true,
  },
];
...
export default function AdminDarzeliai() {
  return (
    <div>
      <div className="mb-6">
        <h1 className="text-lg font-bold text-gray-900">Darželiai</h1>
        <p className="text-sm text-gray-500 mt-1">Tvarkykite darželių sąrašą</p>
      </div>
      <AdminTable
