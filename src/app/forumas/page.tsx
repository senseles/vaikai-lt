...
export default async function ForumPage() {
  ...
  return (
    <>
      {/* Hero */}
      <section className="bg-gradient-to-br from-[#2d6a4f] to-[#40916c] py-12 md:py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 text-center">
          ...
          <Link
            href="/forumas/naujas"
            className="inline-flex items-center gap-2 bg-white text-[#2d6a4f] font-semibold px-6 py-3 rounded-xl hover:bg-green-50 transition-colors shadow-lg"
          >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {categories.map((cat) => (
            <Link
              ...
              className="bg-white dark:bg-slate-800 rounded-xl shadow-md hover:shadow-lg transition-shadow p-5 group border border-gray-100 dark:border-slate-700"
            >
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <h3 className="font-semibold text-slate-800 dark:text-white group-hover:text-[#2d6a4f] dark:group-hover:text-green-400 transition-colors">
                    {cat.name}
                  </h3>
              ...
              {topPosts.map((post) => {
                ...
                return (
                  <Link
                    ...
                    className="flex items-start gap-4 p-4 rounded-xl hover:bg-gray-50 dark:hover:bg-slate-700/50 transition-colors group"
                  >
                    {/* Vote count */}
                    <div className="flex flex-col items-center min-w-[3rem] pt-0.5">
                      <span className={`text-lg font-bold ${score > 0 ? 'text-[#2d6a4f] dark:text-green-400' : score < 0 ? 'text-red-500' : 'text-slate-400'}`}>
                        {score > 0 ? `+${score}` : score}
                      </span>
                      <span className="text-xs text-slate-500 dark:text-slate-400">balsai</span>
                    </div>

                    {/* Content */}
                    <div className="flex-1 min-w-0">
                      <h3 className="font-semibold text-slate-800 dark:text-white group-hover:text-[#2d6a4f] dark:group-hover:text-green-400 transition-colors line-clamp-1">
                        {post.title}
                      </h3>
