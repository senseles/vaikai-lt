  ...
  if (reviews.length === 0) {
    return (
      <div className="text-center py-6">
        ...
        <p className="text-sm text-gray-500 dark:text-gray-400">Atsiliepimu dar nera.</p>
        <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">Bukite pirmas ir pasidalinkite savo patirtimi!</p>
      </div>
    );
  }
      ...
      <div className="mt-2 flex items-center gap-2">
        {reportState === 'idle' && (
          <button
            ...
            title="Pranesti apie atsiliepima"
          >
        ...
        {reportState === 'confirming' && (
          <div className="flex items-center gap-2 text-xs">
            <span className="text-gray-500 dark:text-gray-400">Ar tikrai norite pranesti apie si atsiliepima?</span>
            <button
              onClick={handleReport}
        ...
        {reportState === 'sending' && (
          <span className="text-xs text-gray-400">Siunciama...</span>
        )}

        {reportState === 'sent' && (
          <span className="text-xs text-green-600 dark:text-green-400">Aciu! Jusu pranesimas bus perzeturetas.</span>
        )}

        {reportState === 'error' && (
          <span className="text-xs text-red-500">Nepavyko issiusti. Bandykite veliau.</span>
        )}
      </div>
    </div>
  );
